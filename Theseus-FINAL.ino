...
void Center(int R, int L){
  ...
  if (distanceLEFT < 5) { // getting too close to the right wall, adjust left
    ...
    servoRight.writeMicroseconds(R + 20*scale);
    delay(10);
  } 
  
  if (distanceRIGHT < 5) { // getting too close to the left wall, adjust right
    scale = abs(distanceRIGHT-5)*1.5;
    if (distanceRIGHT < 3){
      scale = 5;
    }
    servoLeft.writeMicroseconds(L - 20*scale);
...
void FWD(uint8_t distanceFront){
  ...
  if (180< distanceFront < 250){
    ...
    servoLeft.writeMicroseconds(fwdL - 170);      // L-ccw
    servoRight.writeMicroseconds(fwdR + 170);     // R-cw
    Center((fwdR + 170), (fwdL - 170));
    delay(10);
  }
  else{
...
void DedN(uint8_t distanceFront){
  while (distanceFront < 140){
    distanceFront = vl.readRange();
    SonarSensor(trigPinLEFT, echoPinLEFT);
    distanceLEFT = distance;
    SonarSensor(trigPinRIGHT, echoPinRIGHT);
    distanceRIGHT = distance;
    servoLeft.writeMicroseconds(rtrnL);
    servoRight.writeMicroseconds(rtrnL);
    delay(50);
  }
  delay(250);
}
